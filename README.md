...
| **👁️ 全能查看器**   | 基于 Three.js + Spark 的高性能 WASM 加速查看器，支持**鼠标 / 触摸 / 键盘(WASD) / 陀螺仪**全模态控制，点击模型聚焦 |
...
├── 📁 static/lib/            # Three.js + Gaussian Splats 3D（Legacy 版本使用）
...
│   ├── 📁 components/
│   │   ├── 📁 common/        # 通用组件 (Button, Modal, Loading, ParticleBackground)
│   │   ├── 📁 gallery/       # 图库组件 (GalleryList, GalleryItem)
│   │   ├── 📁 layout/        # 布局组件 (Sidebar, ControlsBar, TaskQueue, Settings)
│   │   └── 📁 viewer/        # 查看器组件 (ViewerCanvas, VirtualJoystick, GyroIndicator)
│   ├── 📁 hooks/             # 自定义 Hooks (useViewer, useGyroscope, useKeyboard)
